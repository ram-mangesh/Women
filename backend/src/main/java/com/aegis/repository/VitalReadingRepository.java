...
import com.aegis.entity.VitalReading.ReadingSource;
...
public interface VitalReadingRepository extends JpaRepository<VitalReading, Long> {
    ...
    List<VitalReading> findByPatientId(UUID patientId);
    
    List<VitalReading> findByPatientIdAndRecordedAtBetween(
        UUID patientId, LocalDateTime start, LocalDateTime end);
    
    List<VitalReading> findByPatientIdAndSource(UUID patientId, ReadingSource source);
    
    List<VitalReading> findByPatientIdAndIsAbnormal(UUID patientId, Boolean isAbnormal);
    
    List<VitalReading> findByPatientIdAndAlertTriggered(UUID patientId, Boolean alertTriggered);
    
    @Query("SELECT v FROM VitalReading v WHERE v.patient.id = :patientId " +
           "ORDER BY v.recordedAt DESC")
    List<VitalReading> findLatestReadings(@Param("patientId") UUID patientId);
