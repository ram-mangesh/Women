      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: '2rem', fontWeight: 800, color: '#0f172a' }}>
            {data.totalHours}<span style={{ fontSize: '1rem', fontWeight: 400, color: '#64748b' }}> hrs</span>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        {stages.map(s => (
          <div key={s.key} style={{
            ...
            <div style={{ fontSize: '0.65rem', color: '#64748b', marginBottom: 4 }}>{s.icon} {s.label}</div>
            <div style={{ fontSize: '1rem', fontWeight: 700, color: s.color }}>
              {data.stages[s.key].minutes}<span style={{ fontSize: '0.65rem', fontWeight: 400, color: '#64748b' }}> min</span>
            </div>
            <div style={{ fontSize: '0.6rem', color: '#475569' }}>{data.stages[s.key].percentage}%</div>
          </div>
        ))}
      </div>
    </div>
