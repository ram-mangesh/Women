      ...
      <svg width={svgWidth} height={height} viewBox={`0 0 ${svgWidth} ${height}`} style={{ width: '100%', height: 'auto' }}>
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              {/* Bar background */}
              <rect x={x} y={padding.top} width={barWidth} height={chartH} rx={8} fill="#f1f5f9" opacity={1} />
              {/* Bar */}
              <rect
              ...
              {/* Value label */}
              {animated && (
                <text x={x + barWidth / 2} y={y - 6} fontSize={10} fill="#0f172a" textAnchor="middle" fontWeight={600}>
