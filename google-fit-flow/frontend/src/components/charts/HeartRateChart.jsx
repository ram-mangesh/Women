  ...
  return (
    <div style={{ width: '100%', overflowX: 'auto' }}>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto' }}>
        ...
        {yLabels.map(v => (
          <g key={v}>
            <line x1={padding.left} y1={getY(v)} x2={padding.left + chartW} y2={getY(v)} stroke="#e2e8f0" strokeWidth={1} strokeDasharray="4,4" />
            <text x={padding.left - 8} y={getY(v) + 4} fontSize={10} fill="#64748b" textAnchor="end">{v}</text>
          </g>
        ))}
        ...
        {animated && data.length > 0 && (
          <g>
            ...
            <text x={getX(data.length - 1)} y={getY(data[data.length - 1].value) - 12} fontSize={11} fill="#0f172a" textAnchor="middle" fontWeight={700}>
        ...
        {xLabels.map((d, idx) => {
          ...
          return (
            <text key={idx} x={getX(i)} y={height - 5} fontSize={10} fill="#64748b" textAnchor="middle">{d.hour}</text>
          );
        })}
      </svg>
    </div>
